...
module.exports.editProfile = async (req, res) => {
  try {
    ...
    // const isSetterWithLogin = await Setter.findOne({ login: req.body.login, _id: { $ne: req.body.userID } }).maxTimeMS(5000).exec()
    // if (isSetterWithLogin != null) return res.status(403).send({ message: "Пользователь с таким логином уже существует" })

    // const isSetterWithPhone = await Setter.findOne({ phone: req.body.phone, _id: { $ne: req.body.userID } }).maxTimeMS(5000).exec()
